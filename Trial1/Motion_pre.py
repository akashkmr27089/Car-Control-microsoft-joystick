...
class Motion_pre:
    ...
    def input2(self, forw_back, left_right,  emer_on, emer_off):
        self.forw_back = forw_back
        self.left_right = left_right
        print(self.forw_back, self.left_right, self.emergency)
        if(emer_on == 'on' and self.emergency == False):
            self.emergency = True
            print("Emergency stop Recieved ")
            return [0,0,0,0,1]
        elif emer_on == 'off' and emer_off == 'on' and self.emergency == True:
            self.emergency = False
